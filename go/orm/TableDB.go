...
func (backRepoTable *BackRepoTableStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, table *models.Table) (Error error) {
	...
	if tableDB, ok := backRepoTable.Map_TableDBID_TableDB[idx]; ok {
		...
		// 1. reset
		tableDB.TablePointersEncoding.TableRows = make([]int, 0)
		// 2. encode
		for _, tablerowAssocEnd := range table.TableRows {
			tablerowAssocEnd_DB :=
				backRepo.BackRepoTableRow.GetTableRowDBFromTableRowPtr(tablerowAssocEnd)
			
			// the stage might be inconsistant, meaning that the tablerowAssocEnd_DB might
			// be missing from the stage. In this case, the commit operation is robust
			// An alternative would be to crash here to reveal the missing element.
			if tablerowAssocEnd_DB == nil {
				continue
			}
			
			tableDB.TablePointersEncoding.TableRows =
...
func (tableDB *TableDB) DecodePointers(backRepo *BackRepoStruct, table *models.Table) {
	...
	for _, _TableRowid := range tableDB.TablePointersEncoding.TableRows {
		table.TableRows = append(table.TableRows, backRepo.BackRepoTableRow.Map_TableRowDBID_TableRowPtr[uint(_TableRowid)])
	}
